...
from data.dataloader_class import load_data
from model.network_class import create_model, cri_opt_sch
from model.utils_class import train_pt, validate_pt, train_ft, validate_ft, load_pretrained
...
def run_finetune(acc, config, run_name, save_dir, time_now):
    ...
    best_header = [
        ...
        "val_loss","val_acc","val_sens","val_spec","val_auc",
        "model_path","save_dir",
        # "config_json"
    ]
    train_time_header = [
    ...
    # monitor 可用:
    #   - "val_loss", "val_acc", "val_sens", "val_spec", "val_auc"
    #   - 或簡寫 "loss", "acc", "sens", "spec", "auc"
    es_monitor   = es_cfg.get("monitor", "val_loss")
    es_mode      = es_cfg.get("mode", "min")           # loss→min，其餘通常用 max
    ...
    for epoch in range(config['epochs']):
        ...
        val_loss, val_acc, val_sens, val_spec, val_auc = validate_ft(
            model, val_dl, criterion, acc.device, acc
        )
        
        if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
            sch_cfg = (config.get('sch', {}) or {})
            # monitor 可用: loss / acc / sens / spec / auc（或 val_xxx）
            sch_monitor = sch_cfg.get('monitor', 'val_loss')
            if sch_monitor in ["loss", "acc", "sens", "spec", "auc"]:
                sch_monitor = "val_" + sch_monitor

            cur_metrics = {
                "val_loss": float(val_loss),
                "val_acc":  float(val_acc),
                "val_sens": float(val_sens),
                "val_spec": float(val_spec),
                "val_auc":  float(val_auc),
            }
            cur = cur_metrics.get(sch_monitor, float(val_loss))
            scheduler.step(cur)
        ...
        if acc.is_main_process:
            print(
                ...
                f'ACC {val_acc:.6f} | Sens {val_sens:.6f} | '
                f'Spec {val_spec:.6f} | AUC {val_auc:.6f} | LR {lr:.2e}'
            )
    ...
    if acc.is_main_process:
        # 補打一遍 checkpoint 摘要（分類版）
        ckpt = torch.load(f'{save_dir}/model.pt', map_location='cpu')
        print(f"Best val loss : {ckpt.get('val_loss', float('nan')):.6f}")
        if 'val_acc'  in ckpt: print(f"Best val ACC  : {ckpt['val_acc']:.6f}")
        if 'val_sens' in ckpt: print(f"Best val Sens : {ckpt['val_sens']:.6f}")
        if 'val_spec' in ckpt: print(f"Best val Spec : {ckpt['val_spec']:.6f}")
        if 'val_auc'  in ckpt: print(f"Best val AUC  : {ckpt['val_auc']:.6f}")
